...
# ---------------------------------------------------------------------------
# Narration-based direction signals
# ---------------------------------------------------------------------------
# CONSERVATIVE DESIGN: Only include patterns that are structurally embedded
# in Indian bank reference strings (e.g. UPI/CR/..., IMPS/DR/...).  These
# are generated by the NPCI / bank switch and are 100% unambiguous.
#
# DO NOT add generic English words (CREDIT, DEBIT, DEPOSIT, PAYMENT, etc.):
#   - "CREDIT" appears in narrations like "INTEREST CREDIT" (credit) but also
#     in "CREDIT CARD PAYMENT" (debit from account).
#   - "PAYMENT" can be a received payment (credit) or a made payment (debit).
#   - "ATM" is almost always debit but ATM cash loading is credit.
#   - "INT" matches INTEREST (credit), INTERNET CHARGES (debit), INT'L TRANSFER.
# These ambiguities make generic words dangerous for a universal fix.
#
# The /CR/ and /DR/ patterns (slash-bounded) appear in:
#   UPI/CR/123456789, IMPS/CR/123456, NEFT CR / NEFT DR, ACH/CR, etc.
_NARR_CREDIT_RE = re.compile(
    r'('
    r'UPI[/\-]CR[/\-]|'          # UPI/CR/REF or UPI-CR-REF
    r'IMPS[/\-]CR[/\-]|'         # IMPS/CR/REF
    r'NEFT[\s/\-\.]+CR\b|'        # NEFT CR, NEFT/CR, NEFT-CR
    r'RTGS[\s/\-\.]+CR\b|'        # RTGS CR
    r'ACH[/\-]CR\b|'              # ACH/CR
    r'NACH[/\-]CR\b|'             # NACH/CR
    r'COLL[/\-]CR\b|'             # COLLECT/CR
    r'MB[/\-]CR[/\-]|'            # Mobile banking credit ref
    r'IB[/\-]CR[/\-]|'            # Internet banking credit ref
    r'TFR[/\-]CR[/\-]'            # Transfer credit ref
    r')',
    re.IGNORECASE
)
_NARR_DEBIT_RE = re.compile(
    r'('
    r'UPI[/\-]DR[/\-]|'           # UPI/DR/REF or UPI-DR-REF
    r'IMPS[/\-]DR[/\-]|'          # IMPS/DR/REF
    r'NEFT[\s/\-\.]+DR\b|'         # NEFT DR, NEFT/DR, NEFT-DR
    r'RTGS[\s/\-\.]+DR\b|'         # RTGS DR
    r'ACH[/\-]DR\b|'               # ACH/DR
    r'NACH[/\-]DR\b|'              # NACH/DR
    r'MB[/\-]DR[/\-]|'             # Mobile banking debit ref
    r'IB[/\-]DR[/\-]|'             # Internet banking debit ref
    r'TFR[/\-]DR[/\-]|'            # Transfer debit ref
    r'ATM[/\-]WDL[/\-]|'           # ATM Withdrawal (always debit)
    r'WDL[\s][TT]FR[\s]UP[/\-]DR'  # Withdrawal Transfer (from deterministic parser)
    r')',
    re.IGNORECASE
)
...
def generate_zone_candidates(tokens: List[Dict], zone_bounds: Optional[List[float]], zone_type: str) -> List[Dict]:
    if not zone_bounds:
        return []
...
def _extract_block(
    ...
    # --- Phase B: Candidate-Based Zone Reconstruction ---
    ownership = {}
    ...
    # --- Surgical Balance Ownership Recovery Pass ---
    if candidate.get("balance") is None and candidate.get("debit") is not None and candidate.get("credit") is not None:
        rightmost_val = None
        rightmost_x = -1
        has_cr_dr = False
        ...
        for i, tok in enumerate(all_tokens):
            ...
            if val is not None and val > 0:
                if tok.get("x0", -1) > rightmost_x:
                    ...
                    has_cr_dr = False
                    if "CR" in txt.upper() or "DR" in txt.upper():
                        has_cr_dr = True
                    elif i + 1 < len(all_tokens) and all_tokens[i+1].get("text", "").strip().upper() in ("CR", "DR"):
                        has_cr_dr = True

        if has_cr_dr and rightmost_val is not None:
            # Force rightmost value -> balance
            candidate["balance"] = rightmost_val
            
            # Remove ownership from whichever amount column claimed it
            if candidate.get("credit") == rightmost_val:
                candidate["credit"] = None
            elif candidate.get("debit") == rightmost_val:
...
def _detect_zone_swap(blocks: List[List[Dict]], zones: Dict) -> bool:
    """
    Document-level zone-swap detector.

    Samples up to 60 blocks and reads each block's narration text.
    If narration signals say the transaction is a CREDIT but the physical
    token lands in the DEBIT zone (or vice versa) for a clear majority of
    signal-bearing rows, the debit/credit zones are swapped for this PDF.

    Returns True if the zones appear to be swapped.
    """
    debit_zone  = zones.get("debit_zone")
    credit_zone = zones.get("credit_zone")
    if not debit_zone or not credit_zone:
        return False

    agrees   = 0   # narration signal matches the zone that physically holds the amount
    disagrees = 0  # narration signal contradicts the zone

    for block in blocks[:60]:
        # Collect all token text and x positions in the block
        block_text = ""
        for row in block:
            for tok in row.get("tokens", []):
                block_text += " " + tok.get("text", "")
        block_text = block_text.strip()

        # Determine the narration signal (clear CR or DR keyword)
        is_narr_credit = bool(_NARR_CREDIT_RE.search(block_text))
        is_narr_debit  = bool(_NARR_DEBIT_RE.search(block_text))

        if is_narr_credit == is_narr_debit:
            # Both or neither — ambiguous row, skip
            continue

        # Find the physical x position of the amount token (numeric token in DR or CR zone)
        amount_x = None
        for row in block:
            for tok in row.get("tokens", []):
                txt = tok.get("text", "").strip()
        ...
        in_debit  = debit_zone[0]  <= amount_x <= debit_zone[1]
        in_credit = credit_zone[0] <= amount_x <= credit_zone[1]

        if not in_debit and not in_credit:
            continue

        # Does the physical zone agree with the narration signal?
        if is_narr_credit and in_credit:
            agrees += 1
        elif is_narr_debit and in_debit:
            agrees += 1
        elif is_narr_credit and in_debit:
            disagrees += 1
        elif is_narr_debit and in_credit:
            disagrees += 1

    total = agrees + disagrees
    if total < 3:
        return False  # Not enough signal rows to be confident

    swap_ratio = disagrees / total
    logger.info(
        f"V2 ZoneSwapDetector: agrees={agrees} disagrees={disagrees} "
        f"swap_ratio={swap_ratio:.2f}"
    )
    return swap_ratio >= 0.60  # 60%+ disagreement → zones are swapped
...
def parse_with_coordinates(
    ...
    # ── Document-level Zone Swap Detection ───────────────────────────────────
    # Some PDFs (e.g. Bank of India scanned) have their Debit and Credit
    # column headers in the opposite order from what column_detector expects.
    # Sample narration keywords across the document to detect this globally
    # and swap the zones BEFORE any token ownership is assigned.
    zones_swapped = False
    if "debit_zone" in zones and "credit_zone" in zones:
        if _detect_zone_swap(blocks, zones):
            logger.warning(
                "V2: Document-level zone swap detected — "
                f"swapping debit_zone {zones['debit_zone']} ↔ credit_zone {zones['credit_zone']}"
            )
            zones["debit_zone"], zones["credit_zone"] = zones["credit_zone"], zones["debit_zone"]
            zones_swapped = True
    ...
    # ── Direction Correction Pass (Bank-Agnostic) ──────────────────────────────
    # Problem: column_detector may assign debit_zone and credit_zone in the
    # wrong physical positions (e.g. some banks print Withdrawal on the right
    # and Deposit on the left — the reverse of the usual layout).  This causes
    # every transaction to have its debit/credit swapped.
    #
    # Fix: after ledger_truth annotates each row with the EXPECTED direction,
    # we look for rows where:
    #   (a) direction_match is False  — OCR column says debit but math says credit
    #                                   (or vice-versa)
    #   (b) ledger_amount_match is True — the amount itself is correct; only the
    #                                     column label is wrong.
    #
    # When both hold, the fix is unambiguous: swap debit ↔ credit.
    # The raw_extraction values are left unchanged as the ground-truth record;
    # only the canonical debit/credit output fields are corrected.
    #
    # We count corrections and log them so the change is fully observable.
    direction_corrections = 0
    narration_corrections = 0
    ...
    for txn in accepted:
        ...
        if lt.get("available"):
            ...
            if led_dir and led_dir != "none":
                ...
                if not direction_match and amount_match:
                    ...
                    txn["direction_corrected_reason"] = (
                        f"ledger_says_{led_dir}_ocr_said_{ocr_dir}_amount_{led_amt}"
                    )
                    direction_corrections += 1
                    logger.info(
                        f"V2 DirectionFix(math) [{txn.get('date','?')}]: "
                        f"{ocr_dir} → {led_dir}  amount={led_amt}"
                    )
                    continue  # already corrected; skip narration check

        # ── Tier 2: Narration-keyword correction (fallback, no balance needed) ─
        # Only applies when the row was NOT already corrected by Tier 1 AND
        # the row has not already been swapped by the document-level zone swap.
        # Narration signals are unambiguous enough to correct individual rows.
        if txn.get("direction_corrected"):
        ...
        if is_narr_credit == is_narr_debit:
            continue  # ambiguous — do not touch

        narr_dir = "credit" if is_narr_credit else "debit"

        if narr_dir != ocr_dir:
            # Narration clearly disagrees with the column assignment — correct it.
            if ocr_dir == "debit":
                txn["credit"] = txn["debit"]
                txn["debit"]  = None
            else:
            ...
            txn["direction_corrected"]        = True
            txn["direction_corrected_reason"] = (
                f"narration_says_{narr_dir}_ocr_said_{ocr_dir}"
            )
            narration_corrections += 1
            logger.debug(
                f"V2 DirectionFix(narr) [{txn.get('date','?')}]: "
                f"{ocr_dir} → {narr_dir}"
            )

    if direction_corrections or narration_corrections:
        logger.info(
            f"V2 DirectionCorrectionPass: math={direction_corrections} "
            f"narration={narration_corrections} "
            f"total={direction_corrections + narration_corrections} "
            f"out of {len(accepted)} accepted."
        )

    telemetry = {
        "v2_extracted_rows":          len(accepted),
        "v2_rejected_rows":           len(rejected_log),
        "direction_corrections_math": direction_corrections,
        "direction_corrections_narr": narration_corrections,
        "direction_corrections":      direction_corrections + narration_corrections,
        "zones_swapped":              zones_swapped,
