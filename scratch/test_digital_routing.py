...
# 1. Create a truly digital PDF
pdf_path = os.path.join(_workspace, "scratch", "true_digital.pdf")
doc = fitz.open()
page = doc.new_page()
page.insert_text((50, 50), "This is a truly digital PDF with more than 30 words per page to bypass the threshold. " * 5)
doc.save(pdf_path)
doc.close()

# 2. Test detect_document_type as it is in api.py
doc_type, doc_reason = detect_document_type(pdf_path)
print(f"API detect_document_type returned: {doc_type} (Reason: {doc_reason})")

# 3. Test full route_document
t0 = time.time()
