...
def run_experiment():
    ...
    # Test (Heal Before Audit AND APPLY HEAL)
    txns_test = copy.deepcopy(txns)
    sorted_test, _ = validate_and_sort_transactions(txns_test)
    dir_test = run_ledger_direction_validator(sorted_test) # Sets direction_corrected=True
    
    # APPLY THE HEAL TO THE DICTIONARY KEYS!
    ...
    print("\n=== PIPELINE 2: HEAL BEFORE AUDIT + APPLY (Test) ===")
