...
passwords_to_try = set([""])

# All 10 digit numbers in the text
for match in re.findall(r'\b\d{10}\b', text):
    passwords_to_try.add(match)

# Add +91 and 91 prefixes to 10 digit numbers
for match in re.findall(r'\b\d{10}\b', text):
    passwords_to_try.add("91" + match)
    passwords_to_try.add("+91" + match)

# Customer ID
passwords_to_try.add("117074540")
passwords_to_try.add("11707454011")

# Account Number
passwords_to_try.add("150710110010982")

# Phone prefixes from names
# For Bank of India: First 4 characters of name + last 4 digits of registered mobile number
name = "AKSHATA"
for mobile in re.findall(r'\b\d{10}\b', text):
    passwords_to_try.add(name[:4].upper() + mobile[-4:])
    passwords_to_try.add(name[:4].lower() + mobile[-4:])

# Try combinations of tokens
tokens = [t for t in re.findall(r'\w+', text) if len(t) > 3]
for t in tokens:
    passwords_to_try.add(t)
    passwords_to_try.add(t.upper())
    passwords_to_try.add(t.lower())
...
if found_password is not None:
    print(f"PASSWORD_FOUND: {found_password}")
else:
