...
txns, tel = parse_with_coordinates(page_tokens, pdf_name='kotak.pdf', statement_id='123', job_id='456', bank='KOTAK MAHINDRA BANK', pdf_type='DIGITAL')
...
if tel.get('reject_log'):
    ...
    for i, r in enumerate(tel['reject_log'][:5]):
        print(f"Reject {i}: {r['reject_reason']} - {r.get('block_text_snippet', '')}")
